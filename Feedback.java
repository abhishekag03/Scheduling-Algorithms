...
public class Feedback {
	...
	public void method(ArrayList<Process> ProcessList) {
		...
		while (ct < Main.time && ProcessList.size() > 0) {
			//			System.out.println("inf1" + ct + " " + Main.time );
			while (queuecnt<ProcessList.size() && ProcessList.get(queuecnt).arrival_time<ct) {
				SystemQueue.add(ProcessList.get(queuecnt));
				queuecnt++;
		...
		//		ct = 0;queuecnt = 1;
		time_quantum = 8;
		while (ct < Main.time && InteractiveQueue.size()>0) {
			//			System.out.println("inf1" + ct + " " + Main.time );
			while (queuecnt<ProcessList.size() && ProcessList.get(queuecnt).arrival_time<ct) {
				InteractiveQueue.add(ProcessList.get(queuecnt));
				queuecnt++;
				//				System.out.println("inf2");
			}
			int wait_time = 0;
...
//		ct = 0;queuecnt = 1;
		while (ct < Main.time && comp<ProcessList.size()-1) {
			while (queuecnt<ProcessList.size() && ProcessList.get(queuecnt).arrival_time<ct) {
				BatchQueue.add(ProcessList.get(queuecnt));
				queuecnt++;
